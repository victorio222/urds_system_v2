...
const NotificationsDropdown: React.FC<NotificationsDropdownProps> = ({
  ...
  // 🔔 Hardcoded demo notifications (safe for now)
  const notifications = [
    {
      id: 1,
      title: "Research Proposal Submitted",
      message: "Your research proposal was successfully submitted.",
      time: "18:06",
      unread: true,
    },
    {
      ...
      message: "A new research announcement has been posted.",
      time: "13:27",
      unread: false,
    },
    {
      ...
      message: "Your proposal is scheduled for in-house review.",
      time: "11:43",
      unread: false,
    },
  ];
  ...
  return (
    <div
      className={`absolute right-0 mt-3 w-80 rounded-xl shadow-lg bg-white ring-1 ring-slate-200 z-30 transform origin-top-right transition ease-out duration-200 ${transitionClasses}`}
    >
      {/* HEADER */}
      <div className="px-4 py-3 border-b border-gray-100">
        ...
        <div className="flex gap-2 mt-3">
          <span className="px-3 py-1 rounded-full bg-blue-100 text-blue-600 text-xs font-medium cursor-pointer">
            All
          </span>
          <span className="px-3 py-1 rounded-full border text-slate-500 text-xs cursor-pointer hover:bg-gray-100">
            Unread
          </span>
        </div>
      </div>
      ...
      <nav className="ml-auto">
        <ul className="flex items-center gap-4 text-xl">
          ...
          <li className="relative" ref={notifRef}>
            <button
              ...
              className={`p-2 rounded-full hover:bg-gray-100 ${
                isNotifOpen ? "bg-blue-100 text-blue-600" : "text-slate-500"
              }`}
            >
              <BsBellFill />
            </button>
          ...
          <li className="relative" ref={profileRef}>
            <button
              onClick={() => setIsProfileOpen(!isProfileOpen)}
              className={`flex items-center px-2 py-1 rounded-full text-slate-600 hover:text-blue-500 ${
                isProfileOpen ? "text-blue-400" : ""
              }`}
            >
              <span className="text-sm font-semibold pr-3 hidden md:block">
