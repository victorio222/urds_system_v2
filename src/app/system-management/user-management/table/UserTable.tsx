...
// // // --- Type Definitions for API Data and Component Logic ---

// // // 1. User Data Structure (Matches the API response keys)
// // interface UserEntry {
// //   user_id: number;
// //   first_name: string;
// //   last_name: string;
...
// // // 🌟 MAPPED TYPE: Includes 'id' for table tracking and 'display_id' for sequential display.
// // interface TableUserEntry extends UserEntry {
// //   id: number; // Unique identifier (maps to user_id, required for table selection/actions)
// //   display_id: number; // Sequential 1-based index (for visual display)
// // }
...
// // // 4. Table Column Types (Using the broader definition to include control keys)
...
// // // ----------------------------------------------------
// // // 2. User Table Component (Displays data and uses lookup maps)
// // // ----------------------------------------------------
...
// //   collegesMap,
// // }: UserTableProps) => {
// //   // Local UI states (filters, selection)
// //   const [year, setYear] = useState(2025);
// //   const [statusFilter, setStatusFilter] = useState("All Users");
// //   const [selectedRows, setSelectedRows] = useState<number[]>([]);
// //   const [selectAll, setSelectAll] = useState(false);

// //   // Selection logic still uses the unique 'id' (which is the user_id)
...
// //       ),
// //     },
// //     { key: "display_id", header: "ID", align: "center" },
// //     {
// //       key: "first_name",
// //       header: "Name of User",
// //       align: "left",
...
// //     // Use the Maps to display the descriptive names instead of the IDs
// //     {
// //       key: "role_id",
// //       header: "Role",
// //       align: "center",
...
// //             </button>
// //           </div>
// //           {/* Dropdown components are static lists and should be checked for keying internally */}
// //           <Dropdown buttonContent={`Year: ${year}`} items={yearItems} />
// //           <Dropdown
// //             buttonContent={`Status: ${statusFilter}`}
// //             items={statusItems}
// //           />
...
// //       {/* Type Assertion used for external Table component */}
...
// // // ----------------------------------------------------
// // // 1. Parent Component (State and API Manager)
// // // ----------------------------------------------------
...
// //   // Helper to convert an array of API objects into an ID-Name map
// //   const arrayToMap = (
// //     data: any[],
// //     idKey: string,
// //     nameKey: string
...
// //       setCollegesMap(
// //         arrayToMap(collegesRes.data, "college_id", "college_name")
// //       );
// //     } catch (err) {
...
// //   // --- Core Logic: Fetch Users (Updated Mapping) ---
// //   const fetchUsers = async () => {
// //     setFetchStatus("loading");
// //     const token = localStorage.getItem("authToken") || "MOCK_REQUIRED_TOKEN";
...
// //     await fetchLookups(token);

// //     try {
// //       const response: any = await getUsers(token);
// //       let usersArray = response.data.users || response.data || [];
// //       if (!Array.isArray(usersArray)) usersArray = [];

// //       // Map API response to the TableUserEntry structure, setting display_id based on index
// //       const mappedUsers: TableUserEntry[] = usersArray.map(
// //         (user: UserEntry, index: number) => ({
// //           ...user,
// //           id: user.user_id, // Unique ID required by Table component (for tracking)
// //           display_id: index + 1, // Sequential ID starting from 1 (for display)
// //         })
// //       );
...
// //         err.message ||
// //         "Failed to connect to the user API.";
...
// //   // --- Core Logic: Add User (Updated Mapping) ---
...
// //       const response: any = await createUser(token, newUserData);

// //       // MAPPING: Ensure the new object matches the TableUserEntry structure
// //       const newUserFromApi: TableUserEntry = {
// //         user_id: response.data.user_id,
// //         id: response.data.user_id, // Unique ID
// //         display_id: users.length + 1, // Calculate the sequential ID based on the current list length
// //         first_name: response.data.first_name || newUserData.first_name, // Use newUserData properties consistently
...
// //       {/* Pass the dynamic data and the lookup maps to the table */}
// //       <UserTable
// //         data={users}
...
//   return (
//     <form onSubmit={handleSubmit} className="space-y-4 mt-2">
//       {/* Responsive change: Grid stacks on small screens (cols-1) and goes to two columns on medium screens (md:grid-cols-2) */}
...
//       ),
//       align: "center",
//       width: "40px", // Constrain width for responsiveness
//       render: (_: any, row: TableUserEntry) => (
//         <input
//           type="checkbox"
//           checked={selectedRows.includes(row.id)}
...
//       ),
//     },
//     { key: "display_id", header: "ID", align: "center", width: "50px" }, // Constrain width
//     {
//       key: "first_name",
//       header: "Name of User",
//       align: "left",
//       width: "minmax(150px, 2fr)", // Allow more space
//       render: (_: any, row: TableUserEntry) =>
//         `${row.first_name} ${row.last_name}`,
//     },
//     { key: "email", header: "Email", align: "left", width: "minmax(150px, 2fr)" }, // Allow more space

//     // Use the Maps to display the descriptive names instead of the IDs
//     // These columns may be good candidates to hide on very small screens, 
//     // but for now, we keep them and rely on overflow-x-auto.
//     {
//       key: "role_id",
//       header: "Role",
//       align: "center",
...
//     <div className="space-y-4 mt-[-10px] w-full">
//       {/* Filters and Add Button UI */}
//       {/* Responsive change: Use flex-wrap and adjust padding/margin for small screens */}
//       <div className="max-w-full flex justify-end mb-[-5px]">
//         <div className="inline-flex flex-wrap justify-end gap-2 sm:gap-4 bg-white p-2 pr-5 rounded-full shadow">
          
//           <button
//             onClick={() => setModalOpen(true)}
//             className="flex items-center justify-center px-4 py-2 text-sm bg-blue-600 text-white rounded-full hover:bg-blue-700 order-1 md:order-none w-full sm:w-auto" // full width on mobile, then auto
//           >
...
//       {/* Responsive change: Use overflow-x-auto to ensure the table can be scrolled horizontally on small screens */}
...
//         last_name: response.data.last_name || newUserData.last_name,
//         email: response.data.email,
//         // The API response should ideally return all the necessary fields,
//         // but for a smooth UX, we'll try to infer or use defaults if not available.
//         phone_number: response.data.phone_number || null,
//         role_id: response.data.role_id,
//         campus_id: response.data.campus_id,
//         college_id: response.data.college_id,
...
//   return (
//     // Responsive change: Add vertical padding that increases with screen size
